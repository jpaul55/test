  ...
  // Fetch programs from the backend API - only once on component mount
    ...
    const handleSaveProgramIds = async () => {
      try {
        ...
        console.log('Saving Program IDs:', itemsToSave);
        ...
        // Refresh the Program IDs list
        const refreshResponse = await axios.get('/admin/program-ids');
        ...
        const newItemsMessage = newItemsCount > 0
          ? `Added ${newItemsCount} new Program ID${newItemsCount > 1 ? 's' : ''} successfully!`
          : '';

        setSuccessMessage(newItemsMessage || 'Program IDs updated successfully!');
